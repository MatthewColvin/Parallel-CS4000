...
// Modefied by: Matthew Colvin
// Purpose: This code finds whether there exists a Hamiltonian Path 
// from one vertex to another in a graph in parallel using openmp.
//
//
//
//	Origional Author
...
vector<int> Hamiltonian_tour(vector<list<int> >&adj_list, int i1, int j1, bool &found_it) {
  ...
  do {
      if( ( permutiaton % omp_get_num_threads()) == omp_get_thread_num()&& !found_it){ // split up the work between threads
        vector<int> tour;
        tour.resize(n);
        tour[0] = i1;
        tour[n-1] = j1;
      ...
      permutiaton++;    
    } while (next_permutation(perm.begin(),perm.end()) && !found_it);
  }
